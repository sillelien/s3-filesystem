...
RUN apt-get update && apt-get -y install rsync fuse python-pip && pip install yas3fs awscli && \
...
RUN chmod 755 /etc/service/yas3fs/run /etc/cron.daily/backup /etc/cron.monthly/backup /root/update.sh
